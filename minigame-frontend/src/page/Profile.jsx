...
export default function Profile() {
  ...
  const { userId, username, profileIcon, aboutMe, updateProfile, isLoggedIn } =
    useContext(UserContext);
  let params = useParams();

  const [isOwnedAccound, setIsOwnedAccound] = useState(
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      const response = await fetch(
        `${serverURL}/api/public/users/get/${currUserId}`,
        {
          method: "GET",
          headers: {
    ...
    const fetchUserPosts = async () => {
      ...
      const response = await fetch(
        `${serverURL}/api/public/posts/get?userId=${currUserId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
    ...
    fetchUser();
    fetchUserPosts();
  }, [currUserId, isLoggedIn, params.userId, serverURL, userId, editing]);
